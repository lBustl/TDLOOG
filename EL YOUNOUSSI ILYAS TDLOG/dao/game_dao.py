from sqlalchemy import create_engine, Column, Integer, String, ForeignKey, select  # for defining database schema and executing queries
from sqlalchemy.ext.declarative import declarative_base  # base class for defining ORM classes
from sqlalchemy.orm import sessionmaker, relationship  # for creating sessions and defining relationships between ORM classes

# import game-related model classes
...
# define ORM class for representing a game
class GameEntity(Base):
    __tablename__ = 'game'  # specify table name in database
    id = Column(Integer, primary_key=True)  # define ID column as primary key
    players = relationship("PlayerEntity", back_populates="game", cascade="all, delete-orphan")  # define relationship with PlayerEntity

# define ORM class for representing a player
class PlayerEntity(Base):
    __tablename__ = 'player'
    id = Column(Integer, primary_key=True)
    name = Column(String, nullable=False)
    game_id = Column(Integer, ForeignKey("game.id"), nullable=False)  # define foreign key relationship with GameEntity
    game = relationship("GameEntity", back_populates="players")  # define relationship with GameEntity
    battle_field = relationship("BattlefieldEntity", back_populates="player", uselist=False,
                                cascade="all, delete-orphan")  # define relationship with BattlefieldEntity

# define ORM class for representing a battlefield
class BattlefieldEntity(Base):
    ...
    player = relationship("playerEntity", back_populates="player")  # define relationship with PlayerEntity
    vessel = relationship("vesselEntity", back_populates="vessels", cascade="all, delete-orphan")  # define relationship with VesselEntity
...
def map_to_vessel_entity(battlefield_id: int, vessel: Vessel) -> VesselEntity:
    # create a new VesselEntity instance and a new WeaponEntity instance
    vessel_entity = VesselEntity()
    weapon_entity = WeaponEntity()
    
    # set values for the WeaponEntity instance
    ...
    # set values for the VesselEntity instance
...
def map_to_player_entity(player: Player, game_id: int) -> PlayerEntity:
    # create a new PlayerEntity instance
    player_entity = PlayerEntity()
    
    # set values for the PlayerEntity instance
    player_entity.id = player.id
...
def map_to_battlefield_entity(battlefield: Battlefield) -> BattlefieldEntity:
    # create a new BattlefieldEntity instance
    battlefield_entity = BattlefieldEntity()
    
    # set values for the BattlefieldEntity instance
